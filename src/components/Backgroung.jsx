  ...
  return (
    <div className="page fade-up">
      ...
      <div className="container">
        ...
        {/* CTA Buttons & rest of content unchanged */}
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginBottom: 12 }}>
          <button className="btn btn-ghost" style={{ padding: '20px 16px', fontWeight: 700, fontSize: 16, borderRadius: '9999px', height: '68px' }} onClick={() => onTabChange('orders')}>
            📋 I miei ordini
          </button>
          <button className="btn btn-gold" style={{ padding: '20px 16px', fontWeight: 700, fontSize: 16, borderRadius: '9999px', height: '68px' }} onClick={() => onTabChange('shop')}>
            🛍️ SHOP
          </button>
        </div>

        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginBottom: 32 }}>
          <a href="https://t.me/ilrawller" target="_blank" rel="noreferrer" className="btn btn-ghost" style={{ padding: '20px 16px', fontWeight: 700, fontSize: 16, borderRadius: '9999px', textDecoration: 'none', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, height: '68px' }}>
            <span style={{ fontSize: 20 }}>✈️</span> Telegram
          </a>
          <a href="https://instagram.com/therawller" target="_blank" rel="noreferrer" className="btn btn-ghost" style={{ padding: '20px 16px', fontWeight: 700, fontSize: 16, borderRadius: '9999px', textDecoration: 'none', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, height: '68px' }}>
          ...
          <>
            <div style={{ textAlign: 'center', marginBottom: 18 }}>
              <h2 style={{
                ...
                letterSpacing: 5,
                color: '#fff',
                textShadow: `0 0 10px rgba(255,255,255,0.45), 0 0 30px rgba(244,197,66,0.55), 0 0 65px rgba(200,168,75,0.30), 0 0 100px rgba(200,168,75,0.15), 0 4px 12px rgba(0,0,0,0.80)`,
            ...
            <div className="product-grid-3" style={{ marginBottom: 24 }}>
              {featured.map(p => <FeaturedCard key={p.id} p={p} onNavigate={onNavigate} />)}
            </div>
          </>
        )}

        <div className="notice" style={{ marginBottom: 24 }}>
          ⚠️ Ordine minimo €{SHOP_CONFIG.minOrderShipping} per la spedizione tramite corriere. Delivery disponibile solo in Lombardia e Liguria.
        </div>
