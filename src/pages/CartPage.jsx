...
export default function CartPage() {
  ...
  // FIXED VALIDATION - No more "Nome e Cognome" check for Delivery
  const validateRequiredFields = () => {
    if (!isDelivery) return { valid: true };

    // Only Delivery requirements
    if (!address.indirizzo?.trim()) {
      return { valid: false, error: 'Inserisci l\'indirizzo di consegna.' };
    }
    if (!address.telegram || address.telegram.replace('@', '').trim().length === 0) {
      return { valid: false, error: 'Inserisci il tuo username Telegram per completare l\'ordine.' };
    }
    return { valid: true };
  };
  ...
  const handleSubmit = async () => {
    ...
    if (!requiredFieldsValidation.valid) {
      ...
      setError(requiredFieldsValidation.error);
      return;
    }
            ...
            <div className="section-box" style={{ marginTop: 16 }}>
              ...
              <div className="field-group" style={{ marginTop: 8 }}>
                ...
                <input 
                  ...
                  style={attemptedSubmit && !address.indirizzo?.trim() ? { borderColor: 'var(--red)' } : undefined}
                />
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', flex: 1 }}>
                      ...
                      <input
                        ...
                        style={{ 
                          ...
                          marginBottom: 0,
                          flex: 1,
                          borderColor: attemptedSubmit && (!address.telegram || address.telegram.replace('@', '').trim().length === 0) ? 'var(--red)' : undefined 
                        }}
