...
export default function HomePage({ onNavigate, onTabChange }) {
  ...
  const onDragEnd = () => {
    ...
    if (!movedRef.current) {
      // A genuine tap, not a swipe. We deliberately do NOT rely on the
      // native click landing on whatever DOM element the finger happened
      // to be over — cards overlap each other (card width > SPACING), so
      // the front card's box physically sits on top of the cards behind
      // it and would eat the tap in that shared region, making side cards
      // feel unclickable. Instead we work out which card's resting spot
      // is closest to where the finger actually was, independent of which
      // one is visually on top, so every card in the row responds to a
      // tap — front or not.
      const rect = wheelRef.current?.getBoundingClientRect();
      if (rect) {
        const centerX = rect.left + rect.width / 2;
        const tapRelX = lastMoveRef.current.x - centerX;
        let best = null;
        let bestDist = Infinity;
        for (const { p, d } of visibleCards) {
          const dist = Math.abs(d * SPACING - tapRelX);
          if (dist < bestDist) { bestDist = dist; best = { p, d }; }
        }
        if (best) {
          if (Math.round(best.d) === 0) {
            onNavigate('product', best.p);
          } else {
            // d = i + k*n - tick, so centering this exact instance (same i,k)
            // requires tick_new = i + k*n = tick_old + d_old — i.e. ADD d,
            // not subtract it. Subtracting made every tap glide the wrong
            // way, which is why it always seemed to march "forward"
            // regardless of which side you tapped.
            setTick(t => t + best.d);
          }
        }
      }
